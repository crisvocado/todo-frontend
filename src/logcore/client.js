...
export function buildEntry({ severity, message, error, context, labels, fingerprint, config }) {
...
// La firma de estas dos es el contrato del módulo, y client.test.js lo afirma.
// `error` es opcional a propósito: un bug que calcula mal nunca lanza, y
// emitir a mano es la única vía por la que esa clase de fallo llega a logcore.
export function log(severity, message, { error, context, labels, fingerprint } = {}) {
  const config = readConfig()
  if (!config.enabled) return null
  const entry = buildEntry({ severity, message, error, context, labels, fingerprint, config })
  sendEntry(entry, config)
  return entry
}

export function logError(error, { message, context, severity = 'ERROR' } = {}) {
  return log(severity, message ?? String(error?.message ?? error), { error, context })
}
